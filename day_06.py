...
Command = Tuple[str, Tuple[int, int], Tuple[int, int]]

def parse_command(line: str) -> Command:
    if line.startswith("turn"):
        line = line[len("turn")+1:]
    parts = line.split(" ")
    cmd = parts[0]
    x1, y1 = tuple(int(x) for x in parts[1].split(","))
    x2, y2 = tuple(int(x) for x in parts[3].split(","))
    return cmd, (min(x1, x2), min(y1, y2)), (max(x1, x2), max(y1, y2))

def solve(data: str) -> Tuple[str, str]:
    commands = [parse_command(l) for l in data.split("\n")]
    
    lights = defaultdict(lambda: 0)
    for cmd, (x1, y1), (x2, y2) in commands:
        ...
        for x in range(x1, x2+1):
            for y in range(y1, y2+1):
                if cmd == "on":
                    ...
                    lights[(x, y)] += 2
    ans2 = sum(lights.values())

    return str(ans1), str(ans2)

if __name__ == "__main__":
    print(solve("""toggle 461,550 through 564,900
