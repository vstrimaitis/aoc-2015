...
def parse_line(line: str):
    ...
    _, aunt_id = name.split(" ", 1)
    data = dict()
    for part in line.split(", "):
        k, v = part.split(": ")
        data[k] = int(v)
    return int(aunt_id), data
...
def solve(data: str) -> Tuple[str, str]:
    ...
    for line in data.split("\n"):
        id, aunt_data = parse_line(line)
        possible = True
        for k in analysis:
            if k in aunt_data and aunt_data[k] != analysis[k]:
                possible = False
        if possible:
            possible_aunts.append(id)
    assert len(possible_aunts) == 1
    ans1 = possible_aunts[0]
    ...
    for line in data.split("\n"):
        ...
        for k in analysis:
            if k in aunt_data:
                if k in {"cats", "trees"}:
                    if aunt_data[k] <= analysis[k]:
                        possible = False
                elif k in {"pomeranians", "goldfish"}:
                    if aunt_data[k] >= analysis[k]:
                        possible = False
                elif aunt_data[k] != analysis[k]:
                    possible = False
        if possible:
            possible_aunts.append(id)
    assert len(possible_aunts) == 1
    ans2 = possible_aunts[0]
    return str(ans1), str(ans2)

if __name__ == "__main__":
    print(solve("""Sue 1: goldfish: 9, cars: 0, samoyeds: 9
